...
namespace game
{
	void PlatformmerWall::LuaInit(float32 aX, float32 aY, float32 aSizeX, float32 aSizeY)
	{
	...
	void PlatformmerWall::Init(BOOL aActive, const Vector3D<float32> aPosition, const Vector3D<float32> aSize)
	{
		...
		const float32 lWallVertexData[] = {
			...
			lSize.mX, -lSize.mY, -lSize.mZ, lSize.mX, lSize.mZ,
			lSize.mX, -lSize.mY,  lSize.mZ, lSize.mX, 0.0f,
			lSize.mX, -lSize.mY,  lSize.mZ, lSize.mX, 0.0f,
			-lSize.mX, -lSize.mY,  lSize.mZ, 0.0f, 0.0f,
			-lSize.mX, -lSize.mY, -lSize.mZ, 0.0f, lSize.mZ,
	...
	void PlatformmerWall::Update()
	{
		IGameObject::Update();
	}

	void PlatformmerWall::Render()
	{
		IGameObject::Render();
	}

	void PlatformmerWall::Release()
	{
		IGameObject::Release();
	}
