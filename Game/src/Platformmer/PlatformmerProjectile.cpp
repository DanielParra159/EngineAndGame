...
namespace game
{
	void PlatformmerProjectile::Init(const Vector3D<float32> aPosition, const Vector3D<float32> aDirection, BOOL aPlayerProjectile)
	{
	...
	void PlatformmerProjectile::Update()
	{
		IGameObject::Update();
	...
	void PlatformmerProjectile::Render()
	{
		IGameObject::Render();
	}

	void PlatformmerProjectile::Release()
	{
		IGameObject::Release();
	}

	void PlatformmerProjectile::OnCollisionEnter(physics::Collider* other)
	{
		//@TODO: Pool
		logic::World::Instance()->RemoveGameObject(this);
	}
