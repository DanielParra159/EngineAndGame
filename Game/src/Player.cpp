...
namespace game
{
	...
	void Player::Init(BOOL aActive)
	{
		...
		lHead->GetMaterial()->SetDiffuseTextureId(graphics::RenderManager::Instance()->LoadTexture("T_SnakeHead.png"));
		lHead->GetMaterial()->SetColor(&Color(0.5f, 1.0f, 0.5f, 1.0f));
		AddComponent(lHead);
		mTail = graphics::RenderManager::Instance()->LoadMeshFromFile("Box.obj");
		mTail->GetMaterial()->SetDiffuseTextureId(graphics::RenderManager::Instance()->LoadTexture("T_Snake.png"));
